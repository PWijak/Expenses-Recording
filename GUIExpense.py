# UUID is used for defined which data is updated or deleted: https://www.geeksforgeeks.org/generating-random-ids-using-uuid-python/
...
def Save(event = None): #event = None คือการ set ค่า defalut เป็น None
	...
	try :
		...
		transactionid = dt.strftime('%Y%m%d%H%M%f') #%f is micromilisec but %S is sec
		...
		with open('Expense data.csv','a',encoding='utf-8',newline='')  as f: #'a' เพิ่มข้อมูลจากข้อมูลเก่า (Append), 'w' คือจะ save ทับค่าเก่า
			...
			data = [transactionid,date_time,expense,price,piece,overall]
			fw.writerow(data)
...
headerwidth = [120,150,170,80,80,80] #The unit is pixcel
...
def UpdateCSV():
		with open('Expense data.csv','w',newline='',encoding='utf-8') as f: #mode = 'a' = append data / 'w' = replace data
			fw = csv.writer(f) #fr is file reader
			
			data = list(alltransaction.values()) # เตรียมข้อมูลจาก alltrasaction (Dict) ให้กลายเป็น list
			fw.writerows(data) #mutiple line from nested list [[],[],[]] | writerow ต้องรัน for-loop
...
ButtonDelete = ttk.Button(T2,text='Delete',command=DeleteRecord)
ButtonDelete.place(x=50,y=550)
...
def update_table():
	...
	try:
		...
		for d in getdata:
				...
				alltransaction[d[0]] = d #d[0] = transaction id
				resulttable.insert('',0,value=d) #'' = ตัวแรกสุด
		print(alltransaction)
	except Exception as e:
		print('No File')
		print('ERROR:',e)
